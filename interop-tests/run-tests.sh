...
open_eclair_to_ldk_channel() {
  ...
  eclair_wait_for_channel_normal "$eclair_channel_id"
  ...
  ldk_wait_for_channel_usable "$ldk_ucid"
...
test_1_ldk_open_ldk_splice_in() {
  ...
  assert_eq "$mid_value" "500000" "Channel value should not change after only 4 confirmations"
...
test_3_ldk_splice_out() {
  ...
  while true; do
    ...
    if [ -z "$val" ] || [ "$val" = "null" ]; then
      log_fail "LDK channel $ucid no longer exists (force-closed?)"
      return 1
    fi
...
test_4_eclair_splice_out() {
  ...
  while true; do
    ...
    if [ -z "$val" ] || [ "$val" = "null" ]; then
      log_fail "LDK channel $ldk_ucid no longer exists (force-closed?)"
      return 1
    fi
    if [ $(( $(date +%s) - start )) -ge "$timeout" ]; then
